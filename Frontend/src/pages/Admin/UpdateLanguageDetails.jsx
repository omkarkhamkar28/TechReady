...
const UpdateLanguageDetails = () => {
  ...
  const getSingleLanguage = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3001/language/get-single-language/${params.lang}`);
  ...
  const getLanguageNotes = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3001/language-notes/get-languageNotes/${params.lang}`);
      setNotes(data.notes);
    }
    catch (err) {
      console.error(err);
  ...
  const getLanguageMockTest = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3001/language-mocktest/get-mocktest/${params.lang}`);
      ...
      <span className="m-1">
        ...
        <button
          onClick={async () => {
            ...
            try {
              const response = await axios.delete(`http://localhost:3001/language-notes/delete-languageNotes/${id}`);
      ...
      <span className="m-1">
        ...
        <button
          onClick={async () => {
            toast.dismiss(t.id); // Dismiss confirmation toast
            try {
              const response = await axios.delete(`http://localhost:3001/language-mocktest/delete-mocktest/${id}`);
              console.log("Delete Response:", response.data); // Debugging
