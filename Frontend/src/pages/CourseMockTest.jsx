...
const CourseMockTest = () => {
  ...
  const getSingleCourse = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3001/course/get-single-course/${params.course}`
      );
  ...
  const getCourseMockTest = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3001/course-mocktest/get-single-mocktest/${params.mocktest}`
      );
    ...
    <Layout title={`Video - ${courseName}`}>
      <div className="container-fluid my-4">
        ...
        {!examStarted ? (
          ...
          <div>
            <div className="card mx-auto my-4 bg-success text-white shadow-lg" style={{ maxWidth: "400px",backgroundColor:"green" }}>
