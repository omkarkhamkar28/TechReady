...
const Course = () => {
    ...
    const checkAccess = async () => {
        try {
            const { data } = await axios.get(`http://localhost:3001/purchase/check-access/${course}/${name}`);
    ...
    const handlePurchase = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.post(`http://localhost:3001/purchase/purchase-course/${course}/${name}`, {
                course,
                name
            });
